...
namespace TriggerServer
{
	public class NetworkManager
	{
		...
		public void Initialize (Version deviceVersion)
		{
			...
			if (IsHost) {
				...
				//try {
					...
					mainThread = new Thread (ServerReceiveLoop);
				...
				mainThread = new Thread (ClientReceiveLoop);
			}
			...
			while (true) {
				if (client == null || !client.Connected) {
					...
					try {
						...
						client.ReceiveBufferSize = client.SendBufferSize = 64;
			...
			while (true) {
				...
				if (client == null || client.Client == null || !client.Connected || stream == null) {
					try {
						...
						client.Connect (IPAddress, IsRadio ? PORT_RADIO : PORT);
